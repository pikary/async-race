...
function Garage() {
    const { data, currentPage, totalAmount, race } = useTypedSelector(
        (state) => state.cars
    );
    ...
    const handleRaceStart = useCallback(async () => {
        try {
            dispatch(
                createRace({
                    currentPage,
                    cars: data || [createCarWithDefaults()],
                })
            );
            const startEngineRequests = data!.map((car) =>
                dispatch(
                    toggleCarEngineAsync({
                        id: car.id!,
